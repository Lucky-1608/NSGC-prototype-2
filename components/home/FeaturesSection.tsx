...
export function FeaturesSection() {
    return (
        <section className="py-32 relative bg-transparent overflow-hidden" style={{ clipPath: "polygon(0 5%, 100% 0, 100% 95%, 0 100%)" }}>
            ...
            <div className="absolute bottom-1/4 right-[-10%] w-[40%] h-[50%] bg-yellow-600/5 blur-[120px] rounded-full point-events-none" />

            <div className="container mx-auto px-4 lg:px-12 relative z-10 w-full">

                {/* Asymmetric Header */}
                <motion.div
                    className="mb-16 md:w-2/3 lg:w-1/2 border-l-[4px] border-blue-500 pl-6"
                ...
                >
                    ...
                    <h2 className="text-4xl md:text-6xl font-display font-bold uppercase tracking-widest text-white leading-tight mb-6">
                ...
                <motion.div
                    className="flex flex-wrap justify-center lg:justify-end gap-x-8 gap-y-16 pb-24"
                    variants={container}
                    initial="hidden"
                    whileInView="show"
                            ...
                            >
                                <TiltCard className="h-full">
                                    <div className="group relative">
                                        ...
                                        <Card className="h-full relative glass-panel flex flex-col items-start border-l-[3px] border-l-blue-500 hover:border-l-yellow-500 transition-colors duration-500 min-h-[220px]">
                                            <CardHeader className="pb-2">
                                                <div className="w-10 h-10 bg-black border border-blue-500/30 flex items-center justify-center mb-4 text-blue-500 group-hover:text-yellow-500 group-hover:-translate-y-2 group-hover:rotate-12 transition-all duration-300">
                                                    <feature.icon className="w-5 h-5" />
                                                </div>
                                                <CardTitle className="font-display tracking-widest text-lg uppercase bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-500">{feature.title}</CardTitle>
                                            </CardHeader>
                                            <CardContent>
                ...
                <div className="mt-12 md:mt-24 w-full flex flex-col lg:items-end">
                    <motion.div
                        className="text-left lg:text-right mb-12 border-l-[4px] lg:border-l-0 lg:border-r-[4px] border-yellow-500 pl-6 lg:pl-0 lg:pr-6"
                        initial={{ opacity: 0, x: 50 }}
                        whileInView={{ opacity: 1, x: 0 }}
                        viewport={{ once: true }}
                    >
                        <div className="text-[10px] text-yellow-500 font-mono tracking-[0.3em] uppercase mb-4">
                            [ IMMEDIATE // ACCESS ]
                        </div>
                    ...
                    >
                        {quickLinks.map((link, index) => (
                            <motion.div key={link.name} variants={itemVariants} className="animate-float-1" style={{ animationDelay: `${index * 0.5}s` }}>
                                <Link href={link.href}>
                                    <div className="glass-panel border-r-[2px] border-r-yellow-500/50 hover:border-r-yellow-500 hover:-translate-y-2 transition-all duration-300 flex items-center justify-center p-3 sm:p-4 gap-3 cursor-pointer group">
                                        <link.icon className="w-4 h-4 text-gray-500 group-hover:text-yellow-500 transition-colors" />
